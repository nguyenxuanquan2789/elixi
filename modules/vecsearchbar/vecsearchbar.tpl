...
<div class="vec-search-widget">
	<div class="search-container {if $search_type == 'dropdown' || $search_type == 'topbar'}js-dropdown{/if} search-{$search_type}"
		role="search">
		{if $search_type == 'dropdown' || $search_type == 'topbar'}
			...
			<div class="search-content">
				<form class="search-form {if $show_categories && $search_type != 'minimal'}has-categories{/if}"
					action="{$search_controller_url}" data-search-controller-url="{$search_controller_url}"
					method="get">
					<div class="search-input-container">
						...
						{if $show_categories && $search_type != 'minimal'}
							...
							<div class="search-category-items">
								<a href="#" class="search-cat-value" data-id="0">{l s='All categories' mod='vecsearchbar'}<i
										class="elicon-arrow_more"></i></a>
								<ul class="dropdown-search">
									<li><a href="#" class="search-cat-value"
											data-id="0">{l s='All categories' mod='vecsearchbar'}</a></li>
					...
					{if $search_type == 'topbar'}
						<div class="dialog-lightbox-close-button dialog-close-button">
							<i class="elicon-cross" aria-hidden="true"></i>
						</div>
					{/if}
				</form>
							...
							{if $keywords}
								...
								<div class="search-keywords">
									{foreach from=$keywords item="keyword"}
										<a
											href="{$search_controller_url}?controller=search&s={$keyword}"><span>{$keyword}</span></a>
									{/foreach}
								</div>
							{/if}
