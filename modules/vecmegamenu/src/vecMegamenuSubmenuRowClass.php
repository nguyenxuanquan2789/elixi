...
class VecMegamenuSubmenuRowClass extends ObjectModel
{
	const TABLE_NAME = 'vecmegamenu_submenu_row';
	public $id_vecmegamenu_item;
	public $class;
	public $position;
	public $active;
	public static $definition = array(
		'table' => 'vecmegamenu_submenu_row',
		'primary' => 'id_row',
		'fields' => array(
	...
	public function delete()
	{
		...
		if (count($column_items) > 0)
		{
			...
			{
				$res &= $this->deleteSubmenuColumn($column_item['id_vecmegamenu_submenu_column']);
				$res &= $this->deleteSubmenuItem($column_item['id_vecmegamenu_submenu_column']);
			}
		}
		$res &= parent::delete();
		return $res;
	...
	public function getColumByRow($id_row)
	{
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT psc.*
			FROM '._DB_PREFIX_.'vecmegamenu_submenu_column psc
	...
	public function deleteSubmenuItem($id_col)
	{
		...
		$menu_items = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE psi.`id_vecmegamenu_submenu_column` = '.$id_col.' ORDER BY psi.id_vecmegamenu_submenu_item');
	...
	public function deleteSubmenuColumn($id_col)
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'vecmegamenu_submenu_column_lang`
			WHERE `id_vecmegamenu_submenu_column` = '.$id_col
		);
		return $res;
	}
