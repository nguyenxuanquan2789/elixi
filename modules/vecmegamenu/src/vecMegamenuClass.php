...
class VecMegamenuClass extends ObjectModel
{
	const TABLE_NAME = 'vecmegamenu_item';
	...
	public static $definition = array(
		'table' => 'vecmegamenu_item',
		'primary' => 'id_vecmegamenu_item',
		'multilang' => true,
		'multishop' => true,
		'fields' => array(
	...
	public	function __construct($id_vecmegamenu_item = null, $id_lang = null, $id_shop = null, Context $context = null)
	{
		parent::__construct($id_vecmegamenu_item, $id_lang, $id_shop);
	}
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'vecmegamenu_item_shop` (`id_shop`, `id_vecmegamenu_item`)
			VALUES('.(int)$id_shop.', '.(int)$this->id.')'
		);
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'vecmegamenu_submenu` (`id_submenu`,`id_vecmegamenu_item`, `active`)
	...
	public function delete()
	{
		...
		if (preg_match('/sample/', $icon) === 0)
			if ($icon && file_exists(_PS_MODULE_DIR_.'vecmegamenu/views/img/icons/'.$icon))
				$res &= @unlink(_PS_MODULE_DIR_.'vecmegamenu/views/img/icons/'.$icon);
		...
		if (count($row_items) > 0)
		{
			foreach ($row_items as $row_item)
			{
				...
				if (count($column_items) > 0)
				{
					...
					{
						$res &= $this->deleteMenuItem($column_item['id_vecmegamenu_submenu_column']);
						$res &= $this->deleteColumnItem($column_item['id_vecmegamenu_submenu_column']);
					}
				}
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'vecmegamenu_item_shop`
			WHERE `id_vecmegamenu_item` = '.$this->id
		);
		$res &= parent::delete();
		return $res;
	}
	public function deleteSubmenu($id)
	{
		$res = true;
		$res &= Db::getInstance()->execute('
				DELETE FROM `'._DB_PREFIX_.'vecmegamenu_submenu`
				WHERE `id_vecmegamenu_item` = '.$id
			);
		return $res;
	}
	public function getRowByMenu($id_menu)
	{
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT mr.*
			FROM '._DB_PREFIX_.'vecmegamenu_submenu_row mr
			WHERE mr.`id_vecmegamenu_item` = '.$id_menu);
	}
	public function getColumByRow($id_row)
	{
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT mc.*
			FROM '._DB_PREFIX_.'vecmegamenu_submenu_column mc
	...
	public function deleteMenuItem($id_col)
	{
		...
		$menu_items = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE mi.`id_vecmegamenu_submenu_item` = '.$id_col.' ORDER BY mi.id_vecmegamenu_submenu_item');
	...
	public function deleteColumnItem($id_col)
	{
		$res = true;
		$res &= Db::getInstance()->execute('
				DELETE FROM `'._DB_PREFIX_.'vecmegamenu_submenu_column`
				WHERE `id_vecmegamenu_submenu_column` = '.$id_col
			);
		return $res;
	}
	...
	public function deleteRowItem($id_row)
	{
		...
		$res &= Db::getInstance()->execute('
				DELETE FROM `'._DB_PREFIX_.'vecmegamenu_submenu_row`
				WHERE `id_row` = '.$id_row
			);
		return $res;
	}
	...
	public static function getMenus()
	{
		...
		$kq = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			LEFT JOIN `'._DB_PREFIX_.'vecmegamenu_item_lang` pl ON pl.`id_vecmegamenu_item` = pi.`id_vecmegamenu_item`
			LEFT JOIN `'._DB_PREFIX_.'vecmegamenu_item_shop` ps ON ps.`id_vecmegamenu_item` = pi.`id_vecmegamenu_item`
			WHERE pi.active = 1 AND ps.id_shop = '.$id_shop.' AND pl.id_lang='.$id_lang.' ORDER BY pi.position ASC, pi.id_vecmegamenu_item ASC');
		return $kq;
	}
