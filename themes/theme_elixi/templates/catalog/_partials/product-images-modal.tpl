      ...
      <div class="modal-body">
        ...
        <figure>
          {if $product.default_image}
            <img class="js-modal-product-cover product-cover-modal"
              ...
              {/if} height="{$product.default_image.bySize.large_default.height}">
          {else}
        ...
        <div id="thumbnails" class="thumbnails js-thumbnails">
          {block name='product_images'}
            <div
              class="product-images-modal js-modal-product-images slick-block items-desktop-6 items-tablet-6 items-mobile-4"
              data-item="6">
              {foreach from=$product.images item=image}
                <div class="thumb-container js-thumb-container">
                  <img data-image-large-src="{$image.large.url}" class="thumb js-modal-thumb" src="{$image.medium.url}"
                    {if !empty($image.legend)} alt="{$image.legend}" title="{$image.legend}" 
                    {else} alt="{$product.name}"
                    {/if} width="{$image.medium.width}" height="148">
                </div>
              {/foreach}
            </div>
          {/block}
